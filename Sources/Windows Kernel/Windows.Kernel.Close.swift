...
// resolves cross-platform `Kernel.Close` through this alias (the POSIX legs
// resolve theirs via ISO_9945).

#if os(Windows)

extension Windows.Kernel {
    /// Windows CloseHandle-backed close namespace — typealias to the L2-canonical
    /// `Windows.\`32\`.Kernel.Close`.
    public typealias Close = Windows.`32`.Kernel.Close
}
#endif
